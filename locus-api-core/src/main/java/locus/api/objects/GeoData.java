...
public abstract class GeoData extends Storable {
    ...
    /**
     * Check if item is currently visible (this means enabled and visible), so it should be drawn on the map.
     */
    public boolean isVisible() {
        return (state & 1) == 1 // enabled
                && ((state >> 1) & 1) == 1; // visible
    }
    ...
    /**
     * Check if item is currently selected (this means enabled, visible and selected).
     */
    public boolean isSelected() {
        return (state & 1) == 1 // enabled
                && ((state >> 1) & 1) == 1 // visible
                && ((state >> 2) & 1) == 1; // selected
    }
    ...
    /**
     * Get time [ms] when item was created.
     */
    ...
    /**
     * Set new time [ms] when item was created.
     */
